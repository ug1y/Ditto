...
class TestNetwork:
    ...
    def test_mine(self):
        ...
        assert m1.blockdag.graph().nodes().keys() == {1, 2, 4}
        assert m2.blockdag.graph().nodes().keys() == {1, 2, 3, 4}
        assert m2.blockdag.subgraph(4).nodes().keys() == {1, 2, 4}
        assert m2.blockdag.subgraph(3).nodes().keys() == {1, 3}
    ...
    def test_three_miners(self):
        ...
        m3.pre_launch(b1, SimpleRef, CCC)
        ...
        assert m1.blockdag.graph().nodes().keys() == {1, 2, 5}
        assert len(m2.blockdag) == 4

        assert m1.connect_peer(m3.get_name(), 5.0) is True

        assert hash(m2.mine_block()) == 6
        assert m2.blockdag.graph().nodes().keys() == {1, 2, 3, 5, 6}
        assert len(m1.blockdag) == 5

        assert len(m3.blockdag) == 6
        assert m3.blockdag.graph().nodes().keys() == {1, 2, 3, 4, 5, 6}
